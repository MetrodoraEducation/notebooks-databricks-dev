...
# MAGIC %md
# MAGIC Se tiene que gestionar tiempo_de_maduracion (dias entre created_date y fecha inscripción) OK, new_enrollent OK, lead_neto OK, activo (para los registros a eliminar), DimInstitucion (centro que es fijo CESIF para clientify y ISEP para Odoo) OK, estado y stage final (que no son final) ok, ifnull para la fecha incremental OK, gestionar valores null
# MAGIC
...
# MAGIC
# MAGIC          FROM silver_lakehouse.clientifydeals AS clientify_table  where processdate> (select IFNULL(max(fec_procesamiento),'1900-01-01') from silver_lakehouse.sales where sistema_origen='Clientify')
