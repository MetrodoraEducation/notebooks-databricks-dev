...
odoo_df = spark.read.json(f"{bronze_folder_path}/lakehouse/odoo/lead/2024/11/04")
...
# MAGIC %md
# MAGIC **Gestion campos sin stage_id y company_id**

# COMMAND ----------

odoo_df_result = odoo_df.drop("result_company_id","result_stage_id")
...
odoo_df_stage_id = odoo_df.select("result_id","result_stage_id")

# COMMAND ----------

odoo_df_stage_id = odoo_df_stage_id.dropDuplicates()

# COMMAND ----------

odoo_df_stage_id = odoo_df_stage_id.orderBy(odoo_df_stage_id.result_id.desc())

# COMMAND ----------

odoo_df_stage_id = odoo_df_stage_id.withColumn("valuetype",when(col('result_stage_id').rlike('^(\\d)+$'), lit("numeric")).otherwise(lit("non-numeric")))

# COMMAND ----------

odoo_df_stage_id_id = odoo_df_stage_id.filter(odoo_df_stage_id.valuetype == "numeric")

# COMMAND ----------

odoo_df_stage_id_id = odoo_df_stage_id_id.withColumnRenamed("result_id","result_id_stage_id")

# COMMAND ----------

odoo_df_stage_id_id = odoo_df_stage_id_id.drop("valuetype")

# COMMAND ----------

odoo_df_stage_id_value = odoo_df_stage_id.filter(odoo_df_stage_id.valuetype == "non-numeric")

# COMMAND ----------

odoo_df_stage_id_value = odoo_df_stage_id_value.withColumnRenamed("result_stage_id","result_stage_value") \
.withColumnRenamed("result_id","result_id_stage_value")

# COMMAND ----------

odoo_df_stage_id_value = odoo_df_stage_id_value.drop("valuetype")

# COMMAND ----------

odoo_df_result_with_stage_id = odoo_df_result.join(
    odoo_df_stage_id_id, 
    on=odoo_df_result.result_id == odoo_df_stage_id_id.result_id_stage_id, 
    how='outer'
)

# COMMAND ----------

odoo_df_result_with_stage_id_value = odoo_df_result_with_stage_id.join(
    odoo_df_stage_id_value, 
    on=odoo_df_result_with_stage_id.result_id == odoo_df_stage_id_value.result_id_stage_value, 
    how='outer'
)

# COMMAND ----------

odoo_df_result_with_stage_id_value = odoo_df_result_with_stage_id_value.drop("result_id_stage_id","result_id_stage_value")
...
odoo_df_result_with_stage_id_value_with_company_id = odoo_df_result_with_stage_id_value.join(
    odoo_df_company_id_id, 
    on=odoo_df_result_with_stage_id_value.result_id == odoo_df_company_id_id.result_id_company_id, 
    how='outer'
)

# COMMAND ----------

odoo_df_result_with_stage_id_value_with_company_id_value = odoo_df_result_with_stage_id_value_with_company_id.join(
    odoo_df_company_id_value, 
    on=odoo_df_result_with_stage_id_value_with_company_id.result_id == odoo_df_company_id_value.result_id_company_value, 
    how='outer'
)

# COMMAND ----------

odoo_df_final = odoo_df_result_with_stage_id_value_with_company_id_value.drop("result_id_company_id","result_id_company_value")
...
odoo_df_final = odoo_df_final.withColumn(
    "user_id_temp", 
    regexp_replace(split(col("user_id"), ",")[0],"\\[","")
).withColumn(
    "user_value", 
    regexp_extract(split(col("user_id"), ",")[1],'"([^"]*)',1)
...
# MAGIC %md
# MAGIC se necesita cambiar los datatypes (mirar print esquema mas abajo), gestionar el upsert, agregar processdate y sourcesystem 
...
odoo_df_final.createOrReplaceTempView("source_view")

# COMMAND ----------

# MAGIC %sql
# MAGIC MERGE WITH SCHEMA EVOLUTION INTO silver_lakehouse.odoolead
# MAGIC USING source_view 
# MAGIC ON silver_lakehouse.odoolead.id = source_view.id
...
#    .format("delta") \
#    .saveAsTable("silver_lakehouse.odoolead")

# COMMAND ----------

odoo_df_final.printSchema()

# COMMAND ----------

# MAGIC %sql
# MAGIC --select count(*) from silver_lakehouse.odoolead 

# COMMAND ----------

# MAGIC %sql
# MAGIC --select * from silver_lakehouse.odoolead limit 100
